...
const { compressProfilePhoto } = require("../utils/imageCompressor");
    ...
    if (req.file) {
      try {
        ...
        const uploadResult = await uploadImage(compressedBuffer, {
          ...
          mimeType: 'image/jpeg', // Force JPEG after compression
        });

        // Set profile picture URL (store S3 key for presigned URL generation if needed)
        user.profilePicture = uploadResult.secure_url;
        user.profilePictureKey = uploadResult.key; // Store S3 key for future use
        logger.info(
